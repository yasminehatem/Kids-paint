...
void ChangeFillColorAction::ReadActionParameters()
{
	...
	pOut->PrintMessage("Change fill color action: Select the color you want");
...
void ChangeFillColorAction::Execute() 
{
	...
	if (clr != "!")
	{
		pManager->ChangeFillColor(C,clr);   //call function change border color in the App Manager
	}
}
